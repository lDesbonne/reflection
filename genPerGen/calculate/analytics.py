...
topFQuery = Template("SELECT * FROM genPerGen_word WHERE cFem != 0 AND study = $study ORDER BY cFem DESC LIMIT 10")
#Query for top 10 words used by men
topMQuery = Template("SELECT * FROM genPerGen_word WHERE cMale != 0 AND study = $study ORDER BY cMale DESC LIMIT 10")

#TODO finish implementing accuracy updates.
class GenPerGenStats(object):
    
    def __init__(self, study_key, wordlist = []):
        if len(wordlist) > 0:
